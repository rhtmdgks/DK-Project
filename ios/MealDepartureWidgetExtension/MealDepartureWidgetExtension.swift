//
//  MealDepartureWidgetExtension.swift
//  MealDepartureWidgetExtension
//
//  LAON 홈 화면 위젯 4종: 급식 출발 알림, 이번 시간 남은 시간, 오늘의 시간표, 급식 출발 카운트다운.
//
...
struct MealDepartureProvider: TimelineProvider {
    func placeholder(in context: Context) -> MealDepartureEntry { MealDepartureEntry(date: Date()) }
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<MealDepartureEntry>) -> Void) {
        completion(Timeline(entries: [MealDepartureEntry(date: Date())], policy: .atEnd))
    }
}
