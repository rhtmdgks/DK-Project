...
class WeatherNotificationService {
  ...
  static const _scheduledFallbackTitle = '좋은 아침이에요';
  static const _scheduledFallbackBody =
      '오늘 날씨 정보를 준비 중이에요. 앱을 열면 최신 정보를 확인할 수 있어요.';

  static const _errorTitle = '날씨를 잠시 불러오지 못했어요';
  static const _errorBody = '네트워크 상태를 확인한 뒤, 앱에서 다시 한 번 확인해 주세요.';
  ...
  static const _keyNextScheduledDate = 'weather_next_scheduled_date';
  ...
  static Future<void> setWeatherEnabled(
    bool enabled, {
    ...
    if (enabled) {
      if (!skipPermissionCheck) {
        ...
        if (!hasPermission) {
          ...
          if (!granted) {
            await NotificationSettingsRepository.instance.setWeatherEnabled(
              false,
            );
  ...
  static Future<void> maybeShowWeatherNotificationIfNeeded() async {
    final enabled = await NotificationSettingsRepository.instance
        .getWeatherEnabled();
    if (!enabled) return;
    ...
    if (now.hour < 6 || (now.hour == 6 && now.minute < 30)) {
      await _ensureUpcomingSchedule(now, prefs);
      return;
    }
  ...
  static Future<void> _ensureUpcomingSchedule(
    DateTime now,
    SharedPreferences prefs,
  ) async {
    DateTime target = DateTime(now.year, now.month, now.day, 6, 30);
    if (!target.isAfter(now)) {
      target = target.add(const Duration(days: 1));
    }
    final expectedKey =
        '${target.year}-${target.month.toString().padLeft(2, '0')}-${target.day.toString().padLeft(2, '0')}';
    final currentKey = prefs.getString(_keyNextScheduledDate);
